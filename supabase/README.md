...
- `migrations/20260511120000_exercises_only_catalog_nineteen.sql` — в таблице остаются только 19 строк каталога; спина №2 переименована в «Тяга штанги в наклоне»; удалены все пользовательские упражнения и подходы к ним; триггер запрещает `INSERT` с `user_id IS NOT NULL`.
